...
Open http://127.0.0.1:4173/student/. Google sign-in opens the Auth Emulator account picker. Add a synthetic school address. Do not enter a real Google password. New school users default to student. Seed a role for that synthetic account:
    ...
    node --env-file=.env.emulator.example scripts/emulator-role.mjs synthetic-student@tesaban6.ac.th nurse

Allowed roles: student (remove the role), nurse, teacher, admin. Refresh the page to refresh its presentation; the server reads the role on every request. The local web server strips LINE and MQTT environment variables, so SOS/commands report unavailable and cannot reach live recipients or hardware.
