...
//
// ห้ามใส่ emoji (กฎถาวรทุกโปรเจ็ค) — ใช้สีและน้ำหนักตัวอักษรสื่อความหมายแทน
...
// `box1` เป็นชื่อที่เครื่องใช้คุยกัน ไม่ใช่ชื่อที่ครูรู้จัก
...
// ผลจากเซนเซอร์วัดระยะที่ยิงขวางถาดรับของ · แรงกว่า ACK เพราะเป็นของที่เกิดขึ้นทางกายภาพ
// 'unknown' ต้องอ่านออกว่า "ไม่ได้ตรวจ" ไม่ใช่ "ไม่มีของ" ไม่งั้นสายหลุดจะกลายเป็นข้อกล่าวหา
const DROP_TH = Object.freeze({
    confirmed: { label: 'เซนเซอร์เห็นของตกลงถาด', color: THEME.success },
    not_found: { label: 'ไม่เห็นของตกลงมา ช่องอาจหมด ให้ไปดูที่ตู้', color: THEME.danger },
    unknown: { label: 'ตรวจไม่ได้ เซนเซอร์ไม่ตอบ', color: THEME.textMuted }
});
 ...
 *
 * `validateEvent` บังคับว่า `uid` และ `badgeId` ต้องเป็น null เสมอ ⇒ ตู้ยังส่งตัวตนไม่ได้
 * ตรงนี้จึงรับ `student` เข้ามาแล้วแสดงให้ถ้ามี
 ...
 */
function identityBlock(student, hasPhoto = false, verifiedBy = null) {
    const named = typeof student?.name === 'string' && student.name.trim();
    const room = typeof student?.room === 'string' && student.room.trim();
    // บรรทัดรองบอก **ที่มาของชื่อ** ไม่ใช่แค่ชั้นเรียน — ครูต้องรู้ว่าชื่อนี้เชื่อได้แค่ไหน
    // บัตรพิสูจน์แค่ว่ามีคนถือบัตรใบนั้น ส่วนบัญชีโรงเรียนผ่านการยืนยันโทเคนฝั่งเซิร์ฟเวอร์
    const subtitle = named
        ? (verifiedBy === 'school_account'
            ? (room ? `ชั้น ${room} · ลงชื่อเข้าใช้ด้วยบัญชีโรงเรียน` : 'ลงชื่อเข้าใช้ด้วยบัญชีโรงเรียน')
            : room ? `ชั้น ${room}` : 'ไม่ระบุชั้นเรียน')
        : verifiedBy === 'school_account' ? 'สั่งจากเว็บด้วยบัญชีโรงเรียน แต่ยังดึงชื่อไม่ได้'
        : hasPhoto ? 'ใช้ตู้โดยไม่มีบัตร — ดูรูปด้านบนว่าเป็นใคร'
    ...
    return {
        type: 'box', layout: 'vertical', spacing: 'xs', contents: [
            ...
            text(subtitle, { size: 'sm', color: THEME.textMuted })
        ]
    };
}

function shell({ title, accent, body, origin, photoUrl }) {
    const bubble = {
        type: 'bubble',
        header: {
    ...
    // รูปใบหน้าของคนที่ใช้ตู้โดยไม่มีบัตร — ลิงก์อายุ 15 นาที
    // ถ้าครูเปิดแชตช้ากว่านั้น รูปจะแตก ซึ่งเป็นผลที่ตั้งใจ ไม่ใช่ความผิดพลาด
    // LINE ต้องการ https และดึงรูปเอง จึงตรวจรูปแบบก่อน ไม่ปล่อยค่าแปลกปลอมเข้าไป
    if (typeof photoUrl === 'string' && /^https:\/\/[a-z0-9.-]+\/[^\s"']*$/i.test(photoUrl)) {
        bubble.hero = {
            type: 'image', url: photoUrl, size: 'full', aspectRatio: '4:3', aspectMode: 'cover'
        };
    }
    if (origin) {
        bubble.footer = {
...
export function sosBubble(event, { student = null, origin = '', photoUrl = null } = {}) {
    const body = [identityBlock(student, !!photoUrl), separator,
    ...
    return shell({ title: 'เรียกครูพยาบาล', accent: THEME.danger, body, origin, photoUrl });
}

/** ข้อความแจ้งการใช้ตู้ — ไม่ใช่เรื่องด่วน แต่ครูต้องเห็นว่าใครใช้อะไรไป */
export function dispenseBubble(event, { student = null, origin = '', photoUrl = null } = {}) {
    const ack = ACK_TH[event.ack] || { label: event.ack, color: THEME.textMuted };
    const drop = DROP_TH[event.dropCheck] || DROP_TH.unknown;
    const body = [identityBlock(student, !!photoUrl, event.verifiedBy), separator,
        ...
        row('ของตกถาด', drop.label, drop.color),
        text('เซนเซอร์บอกได้ว่ามีของตกลงถาด แต่ยืนยันไม่ได้ว่าเป็นของถูกชนิด และไม่ได้บอกว่านักเรียนหยิบไปแล้ว',
            { size: 'xs', color: THEME.textMuted })];
    if (event.clockTrust && event.clockTrust !== 'ntp') {
        body.push(text(CLOCK_WARNING, { size: 'xs', color: THEME.textMuted }));
    }
    return shell({ title: 'มีการใช้ตู้ปฐมพยาบาล', accent: THEME.primary, body, origin, photoUrl });
}
