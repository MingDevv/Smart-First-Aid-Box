...
export function sosBubble(event, { student = null, origin = '' } = {}) {
    const body = [identityBlock(student), separator,
        row('ที่ตู้', event.cabinetId === 'web' ? 'กดจากเว็บ ไม่ได้กดที่ตู้' : `ตู้ ${event.cabinetId}`),
...
export function altTextFor(event, student = null) {
    ...
    return event.kind === 'sos'
        ? `เรียกครูพยาบาล — ${who} ที่ ${event.cabinetId === 'web' ? 'เว็บ' : `ตู้ ${event.cabinetId}`} ${formatThaiTime(event.ts)}`
