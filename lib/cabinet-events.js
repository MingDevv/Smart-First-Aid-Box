import { EVENT_ID, ACCOUNT_UID } from './cabinet-protocol.js';
...
export function validateEvent(value, cabinetId) {
    if (!value || typeof value.id !== 'string' || !EVENT_ID.test(value.id) || value.cabinetId !== cabinetId ||
        ...
        !(value.uid === null ||
            value.kind === 'dispense' && value.verifiedBy === 'cabinet_card' && value.uid === value.studentId ||
            value.kind === 'dispense' && value.verifiedBy === 'school_account' && typeof value.uid === 'string' && ACCOUNT_UID.test(value.uid)) || typeof value.historical !== 'boolean' ||
    ...
    // `school_account` = สั่งจากเว็บหลังยืนยันโทเคนบัญชีโรงเรียนฝั่งเซิร์ฟเวอร์แล้ว
    // ตัวตนแรงกว่าบัตร (บัตรพิสูจน์แค่การถือบัตร) แต่ไม่มี studentId ของทะเบียนติดมา
    // ชื่อถูกแปลงจาก uid ตอนจะส่ง LINE ไม่ได้เดินทางมากับเหตุการณ์
    const account = value.badgeId === null && value.studentId == null &&
        value.verifiedBy === 'school_account' && typeof value.uid === 'string';
    if (![1, 2].includes(value.drawer) || !(identified || anonymous || account) ||
    ...
    // หลักฐานจากเซนเซอร์วัดระยะที่ยิงขวางถาดรับของ · **สามค่าเท่านั้น**
    // `unknown` = เซนเซอร์ไม่ตอบ/ไม่มีค่าถาดว่าง ซึ่งไม่ใช่ `not_found` และห้ามยุบรวมกัน
    // เหตุการณ์ที่ตู้ส่งมาก่อนมีเซนเซอร์ไม่มีฟิลด์นี้ ⇒ ตกไปที่ `unknown` ไม่ใช่ error
    if (value.dropCheck !== undefined && value.dropCheck !== null &&
        !['confirmed', 'not_found', 'unknown'].includes(value.dropCheck)) throw invalid();
    return { ...event, drawer: value.drawer, ...(identified ? { studentId: value.studentId } : {}), badgeId: value.badgeId, verifiedBy: value.verifiedBy,
        ...
        dropCheck: value.dropCheck ?? 'unknown' };
}
