...
export function validateEvent(value, cabinetId) {
    if (!value || typeof value.id !== 'string' || !EVENT_ID.test(value.id) || value.cabinetId !== cabinetId ||
        ...
        value.uid !== null || typeof value.historical !== 'boolean' ||
        !['ntp', 'rtc', 'untrusted'].includes(value.clockTrust)) throw invalid();
    const event = { id: value.id, kind: value.kind, cabinetId, uid: null, ts: value.ts,
    ...
    if (value.kind === 'sos') {
        ...
        return { ...event, buzzerAck: value.buzzerAck };
    }
    if (![1, 2].includes(value.drawer) || value.badgeId !== null || value.verifiedBy !== 'unidentified' ||
    ...
    return { ...event, drawer: value.drawer, badgeId: null, verifiedBy: 'unidentified',
