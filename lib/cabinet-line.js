...
// ปุ่ม "ดูประวัติการใช้ตู้" ต้องชี้กลับมาที่เว็บของเราเอง · บน Vercel ค่านี้มีให้อยู่แล้ว
export function publicOrigin(env = process.env) {
    const raw = env.SFAB_PUBLIC_ORIGIN || (env.VERCEL_PROJECT_PRODUCTION_URL ? `https://${env.VERCEL_PROJECT_PRODUCTION_URL}` : '');
    return /^https:\/\/[a-z0-9.-]+$/i.test(raw.replace(/\/+$/, '')) ? raw.replace(/\/+$/, '') : '';
}

export async function deliverEvent(db, event, { env = process.env, send = pushLine, now = Date.now, resolve = resolveStudent } = {}) {
    ...
    const claim = await db.runTransaction(async tx => {
        ...
        const payload = state.payload || { to: env.LINE_GROUP_ID.trim(), messages: [lineMessage(event, { student, origin: publicOrigin(env) })] };
