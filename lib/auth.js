...
// โรงเรียนนี้มีสามบทบาทเท่านั้น: admin, teacher, student (Bank 2026-09-14)
//
// `nurse` ถูกยกเลิก — ครูกับผู้ดูแลระบบทำงานทุกอย่างเหมือนกัน จึงไม่มีเหตุให้แยกบทบาทที่สาม
// เอกสาร `roles/` ที่ยังเขียนว่า nurse ค้างอยู่จะตกกลับเป็น `student` โดยบรรทัด `STAFF_ROLES.includes`
// ด้านล่าง ซึ่งคือการ fail closed ที่ถูกต้อง · ตรวจแล้วว่า production ไม่มีเอกสารแบบนั้นเลย
// (2026-09-14, `roles` มีฉบับเดียวคือ Bank/admin) ⇒ ไม่มีใครถูกตัดสิทธิ์จากการเปลี่ยนนี้
//
// `CLINICAL_ROLES` เท่ากับ `STAFF_ROLES` มาตั้งแต่ WP2 และคงไว้เป็นชื่อเรียกเดิมของฝั่งที่อ้างถึงมัน
export const STAFF_ROLES = Object.freeze(['teacher', 'admin']);
