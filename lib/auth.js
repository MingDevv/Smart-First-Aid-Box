...
// โรงเรียนนี้มีสามบทบาทเท่านั้น: admin, teacher, student
//
...
// ด้านล่าง ซึ่งคือการ fail closed ที่ถูกต้อง
//
// `CLINICAL_ROLES` เท่ากับ `STAFF_ROLES` และคงไว้เป็นชื่อเรียกเดิมของฝั่งที่อ้างถึงมัน
...
export function createAuthorizer(services = firebaseServices) {
    return async function authorize(req, { staffOnly = false } = {}) {
        ...
        let token;
        try {
            // ตรวจบัญชีที่ถูกปิดและสิทธิ์ที่ถูกถอนด้วย สิทธิ์ที่หน้าเว็บจำไว้ต่ออายุให้ไม่ได้
