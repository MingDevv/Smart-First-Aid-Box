...
| `<base>/evt` | ESP32 | ไม่ | `{"event":"door_open","ts":...}` |
...
> มีด่านกันอีกสองชั้น (ทิ้งข้อความที่เข้ามาใน 2 วินาทีแรกหลัง subscribe และทิ้งคำสั่งที่เก่ากว่า 30 วินาที)
...
| `vercel-publisher` | publish บน `crms6/firstaidbox/box1/cmd` | Vercel (`api/command.js`) |
...
เฟิร์มแวร์ยังเปิด HTTP endpoint เดิมไว้ครบ (`/status`, `/open`, `/buzzer`) และหน้าเว็บจะ
...
การเชื่อมต่อ MQTT เขียนแบบไม่บล็อก ถ้า broker ล่ม ESP32 จะไม่ค้างรอ — เว็บเซิร์ฟเวอร์
ในตัวมันยังตอบคำสั่งจากวง LAN ได้ตามปกติ
